...
} from '../../src/headless/seed-words.js';
...
describe('shuffleWithSeed', () => {
  it('is deterministic: same seed produces same order', () => {
    ...
    const r1 = shuffleWithSeed(items, 42);
    const r2 = shuffleWithSeed(items, 42);
    expect(r1).toEqual(r2);
  });

  it('produces different orders for different seeds', () => {
    const items = ['a', 'b', 'c', 'd', 'e'];
    const r1 = shuffleWithSeed(items, 1);
    const r2 = shuffleWithSeed(items, 2);
    expect(r1).not.toEqual(r2);
  });
});

describe('buildVerificationGrid', () => {
  it('places all real words in the grid with correct indices', () => {
    const realWords = ['abandon', 'ability', 'able', 'about', 'above'];
    const grid = buildVerificationGrid(realWords, 8, 42, TEST_WORDLIST);

    expect(grid.words.length).toBe(8);
    for (const w of realWords) {
      expect(grid.words).toContain(w);
    }
    expect(grid.correctIndices.length).toBe(5);
    for (const idx of grid.correctIndices) {
      expect(grid.words[idx]).toBeOneOf(realWords);
    }
  });

  it('all realWords are present and correctIndices point to them', () => {
    const realWords = ['abandon', 'ability', 'able'];
    const grid = buildVerificationGrid(realWords, 6, 7, TEST_WORDLIST);

    for (const idx of grid.correctIndices) {
      expect(realWords).toContain(grid.words[idx]);
    }
  });

  it('throws when gridSize < realWords.length', () => {
    const realWords = ['abandon', 'ability', 'able'];
    expect(() =>
      buildVerificationGrid(realWords, 2, 42, TEST_WORDLIST)
    ).toThrow('gridSize');
  });

  it('decoys do not duplicate or overlap with realWords', () => {
    const realWords = ['abandon', 'ability'];
  ...
  it('12-word grid', () => {
    ...
    const grid = buildVerificationGrid(realWords, 20, 123, TEST_WORDLIST);

    expect(grid.words.length).toBe(20);
    expect(grid.correctIndices.length).toBe(12);
    for (const w of realWords) {
      expect(grid.words).toContain(w);
    }
  });
...
describe('isGridSelectionCorrect', () => {
  const grid = {
  ...
  it('returns false for extra selection', () => {
    expect(isGridSelectionCorrect(grid, [0, 3, 5, 1])).toBe(false);
  });
