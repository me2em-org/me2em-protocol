// packages/react/test/headless/verify-flow.spec.ts
import { describe, it, expect, vi } from 'vitest';
import {
  verifyFlowReducer,
  initVerifyState,
} from '../../src/headless/verify-flow.js';

const TEST_WORDLIST = [
  'abandon', 'ability', 'able', 'about', 'above', 'absent',
  'absorb', 'abstract', 'absurd', 'abuse', 'access', 'accident',
  'account', 'accuse', 'achieve', 'acoustic', 'acquire', 'across',
  'act', 'action', 'actor', 'actress', 'actual', 'adapt',
  'add', 'addict', 'address', 'adjust', 'admit', 'adult',
  ...
  'arrange', 'arrive', 'arrow', 'art', 'artefact', 'artist',
  'artwork', 'ask', 'aspect', 'assault', 'asset', 'assist',
  ...
  'boring', 'borrow', 'boss', 'bottom', 'bounce', 'brain',
  ...
  'bulk', 'bullet', 'bundle', 'bunny', 'burden', 'burger',
  ...
  'chaos', 'chapter', 'charge', 'chase', 'cheap', 'check',
  ...
  'cruise', 'crumble', 'crush', 'cry', 'crystal', 'cube',
  ...
  'fancy', 'fantasy', 'farm', 'fashion', 'fat', 'fatal',
  'father', 'fatigue', 'fault', 'favorite', 'feature', 'february',
  'federal', 'fee', 'feed', 'feel', 'female', 'fence',
  'festival', 'fetch', 'fever', 'few', 'fiber', 'fiction',
  'field', 'figure', 'file', 'film', 'filter', 'final',
  ...
  'latin', 'laugh', 'laundry', 'lava', 'lawn', 'lawsuit',
  'layer', 'lazy', 'leader', 'leaf', 'learn', 'leave',
  'lecture', 'left', 'leg', 'legal', 'legend', 'leisure',
  'lemon', 'lend', 'length', 'lens', 'leopard', 'lesson',
  'letter', 'level', 'liberty', 'library', 'license', 'life',
  ...
  'meadow', 'mean', 'measure', 'meat', 'mechanic', 'media',
  ...
  'notable', 'nothing', 'notice', 'novel', 'now', 'nuclear',
  ...
  'omit', 'once', 'one', 'onion', 'online', 'open',
  'opinion', 'oppose', 'option', 'orange', 'orbit', 'orchard',
  'order', 'ordinary', 'organ', 'orient', 'original', 'orphan',
  'ostrich', 'other', 'outdoor', 'outer', 'output', 'outside',
  'oval', 'oven', 'over', 'own', 'owner', 'oxygen',
  'oyster', 'ozone', 'pact', 'paddle', 'pad', 'page',
  ...
  'simple', 'since', 'sing', 'siren', 'sister', 'sit',
  'situation', 'six', 'size', 'skate', 'sketch', 'ski',
  'skill', 'skin', 'skirt', 'skull', 'slab', 'slam',
  'sleep', 'slender', 'slice', 'slide', 'slight', 'slim',
  'slogan', 'slot', 'slow', 'slush', 'small', 'smart',
  'smile', 'smoke', 'smooth', 'snack', 'snake', 'snap',
  'sniff', 'snow', 'soap', 'soccer', 'social', 'sock',
  'soda', 'soft', 'solar', 'soldier', 'solid', 'solution',
  'solve', 'someone', 'song', 'soon', 'sorry', 'sort',
  'soul', 'sound', 'soup', 'source', 'south', 'space',
  'spare', 'spatial', 'spawn', 'speak', 'special', 'speed',
  'spell', 'spend', 'sphere', 'spice', 'spider', 'spike',
  'spin', 'spirit', 'split', 'sponsor', 'spoon', 'sport',
  ...
  'submit', 'subtle', 'success', 'such', 'sudden', 'suffer',
  ...
  'uniform', 'union', 'unique', 'unit', 'universe', 'unknown',
  'unlock', 'until', 'unusual', 'unveil', 'update', 'upgrade',
  'uphold', 'upon', 'upper', 'upset', 'urban', 'usage',
...
describe('verifyFlowReducer', () => {
  const realWords = ['abandon', 'ability', 'able', 'about', 'above'];

  it('init builds grid of correct size', () => {
    const state = initVerifyState(realWords, 8, 42, TEST_WORDLIST);
    expect(state.grid.words.length).toBe(8);
    expect(state.selected).toEqual([]);
    expect(state.status).toBe('collecting');
    expect(state.attempts).toBe(0);
  });

  it('SELECT adds index to selected', () => {
    let state = initVerifyState(realWords, 8, 42, TEST_WORDLIST);
    state = verifyFlowReducer(state, { type: 'SELECT', index: 3 });
    expect(state.selected).toEqual([3]);
  });

  it('DESELECT removes index from selected', () => {
    let state = initVerifyState(realWords, 8, 42, TEST_WORDLIST);
    state = verifyFlowReducer(state, { type: 'SELECT', index: 3 });
    state = verifyFlowReducer(state, { type: 'DESELECT', index: 3 });
    expect(state.selected).toEqual([]);
  });

  it('double SELECT warns and keeps state', () => {
  ...
  it('CHECK with wrong count warns and stays collecting', () => {
    const spy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    let state = initVerifyState(realWords, 8, 42, TEST_WORDLIST);
    state = verifyFlowReducer(state, { type: 'SELECT', index: 0 });
    state = verifyFlowReducer(state, { type: 'CHECK' });
    expect(state.status).toBe('collecting');
    spy.mockRestore();
  });

  it('CHECK with correct in-order selection → correct', () => {
    let state = initVerifyState(realWords, 8, 42, TEST_WORDLIST);
    for (const idx of state.grid.correctIndices) {
      state = verifyFlowReducer(state, { type: 'SELECT', index: idx });
    }
    state = verifyFlowReducer(state, { type: 'CHECK' });
  ...
  it('CHECK with wrong order → incorrect', () => {
    const grid = initVerifyState(realWords, 8, 42, TEST_WORDLIST);
    const wrongIndices = [...grid.grid.correctIndices].reverse();
    let state = grid;
  ...
  it('after correct: SELECT is ignored', () => {
    const spy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    let state = initVerifyState(realWords, 8, 42, TEST_WORDLIST);
    for (const idx of state.grid.correctIndices) {
      state = verifyFlowReducer(state, { type: 'SELECT', index: idx });
  ...
  it('RESET reinitializes with new params', () => {
    let state = initVerifyState(realWords, 8, 42, TEST_WORDLIST);
    state = verifyFlowReducer(state, {
      type: 'RESET',
      realWords: ['abandon', 'about'],
      gridSize: 5,
      randomSeed: 99,
      allWords: TEST_WORDLIST,
    });
    expect(state.grid.words.length).toBe(5);
    expect(state.selected).toEqual([]);
    expect(state.status).toBe('collecting');
    expect(state.attempts).toBe(0);
  });

  it('repeated words in phrase select via distinct cells', () => {
    const dupWords = ['abandon', 'ability', 'abandon'];
    let state = initVerifyState(dupWords, 6, 42, TEST_WORDLIST);
    // The correct indices are distinct for each occurrence
    const correct = state.grid.correctIndices;
    expect(correct[0]).not.toBe(correct[2]);
